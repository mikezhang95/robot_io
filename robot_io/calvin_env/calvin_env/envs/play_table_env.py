...
class PlayTableSimEnv(gym.Env):
    ...
    def reset(self, robot_obs=None, scene_obs=None, 
            target_pos=None, target_orn=None, gripper_state=None):
        
        if target_pos is not None:
            gripper_state = 1 if gripper_state == 'open' else -1
            self.robot.apply_action([target_pos, target_orn, gripper_state])
        else:
    ...
    def get_state_obs(self):
        ...
        robot_info['joint_positions'] = robot_info['arm_joint_states'][-8:-1]
        scene_obs = self.scene.get_obs()
        # obs = {"robot_obs": robot_obs, "scene_obs": scene_obs}
        obs = {"robot_state": robot_info, "scene_obs": scene_obs}
        return obs
